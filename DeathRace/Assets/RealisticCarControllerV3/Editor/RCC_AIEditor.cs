...
[CustomEditor(typeof(Death_AICarController))]
public class RCC_AIEditor : Editor {

	Death_AICarController aiController;
	...
	static void CreateAIBehavior(){
		...
		if (Selection.activeGameObject.GetComponentInParent<Death_AICarController> ()) {
			EditorUtility.DisplayDialog("Your Vehicle Already Has AI Car Controller", "Your Vehicle Already Has AI Car Controller", "Ok");
			return;
		}

		Selection.activeGameObject.GetComponentInParent<Death_Race_CarControllerV3> ().gameObject.AddComponent<Death_AICarController>();
		GameObject car = Selection.activeGameObject.GetComponentInParent<Death_Race_CarControllerV3> ().gameObject;
	...
	static void CreateWaypointsContainer(){
		...
		wp.AddComponent<Death_Race_AIWaypointsContainer>();
	...
	public override void OnInspectorGUI () {
		...
		aiController = (Death_AICarController)target;

		if(!aiController.gameObject.GetComponent<Death_Race_CarControllerV3>().AIController)
			aiController.gameObject.GetComponent<Death_Race_CarControllerV3>().AIController = true;
...
//			aiController.gameObject.GetComponent<RCC_CarControllerV3>().autoReverse = true;
		...
		if(aiController._AIType == Death_AICarController.AIType.FollowWaypoints)
		...
		if(aiController.limitSpeed)
			EditorGUILayout.Slider(serializedObject.FindProperty("maximumSpeed"), 0f, aiController.GetComponent<Death_Race_CarControllerV3>().maxspeed);
