...
[CustomEditor(typeof(Death_Race_CarControllerV3)), CanEditMultipleObjects]
public class RCC_Editor : Editor {

	Death_Race_CarControllerV3 carScript;
	...
	static void CreateBehavior(){
		...
		if(!Selection.activeGameObject.GetComponentInParent<Death_Race_CarControllerV3>()){
			...
			pivot.AddComponent<Death_Race_CarControllerV3>();
	...
	public override void OnInspectorGUI () {
		...
		carScript = (Death_Race_CarControllerV3)target;
		defBackgroundColor = GUI.backgroundColor;

		if(!carScript.GetComponent<Death_AICarController>())
...
//			EditorGUILayout.PropertyField(serializedObject.FindProperty("autoReverse"), new GUIContent("Auto Reverse", "Enables/Disables auto reversing when player press brake button. Useful for if you are making parking style game."));
			...
			if(carScript._wheelTypeChoise == Death_Race_CarControllerV3.WheelType.BIASED){
		...
		if(carScript.GetComponent<Death_AICarController>()){
			...
			if(GUILayout.Button("Remove AI Controller From Vehicle")){
				carScript.AIController = false;
				DestroyImmediate(carScript.GetComponent<Death_AICarController>());
			}
		...
		if(carScript.AIController && !GameObject.FindObjectOfType<Death_Race_AIWaypointsContainer>())
