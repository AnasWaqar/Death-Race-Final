...
public class RCC_EnterExitCar : MonoBehaviour {
	...
	void Awake (){
		...
		carCamera = GameObject.FindObjectOfType<Death_Race_Camera>().gameObject;
	...
	void GetIn (){
		...
		if(carCamera.GetComponent<Death_Race_Camera>()){
			carCamera.GetComponent<Death_Race_Camera>().cameraSwitchCount = 10;
			carCamera.GetComponent<Death_Race_Camera>().ChangeCamera();
		}

		carCamera.transform.GetComponent<Death_Race_Camera>().SetPlayerCar(gameObject);
