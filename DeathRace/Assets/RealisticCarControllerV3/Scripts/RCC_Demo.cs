...
public class RCC_Demo : MonoBehaviour {
	...
	public Death_Race_CarControllerV3[] selectableVehicles;
	...
	public void Spawn () {
		...
		Death_Race_CarControllerV3[] activeVehicles = GameObject.FindObjectsOfType<Death_Race_CarControllerV3>();
		...
		if(activeVehicles != null && activeVehicles.Length > 0){
			...
			foreach(Death_Race_CarControllerV3 rcc in activeVehicles){
		...
		if(lastKnownPos == Vector3.zero){
			if(	GameObject.FindObjectOfType<Death_Race_Camera>()){
				lastKnownPos = GameObject.FindObjectOfType<Death_Race_Camera>().transform.position;
				lastKnownRot = GameObject.FindObjectOfType<Death_Race_Camera>().transform.rotation;
			}
		}
		...
		newVehicle.GetComponent<Death_Race_CarControllerV3>().canControl = true;

		// Setting new target of RCC Camera to our new car.
		if(	GameObject.FindObjectOfType<Death_Race_Camera>()){
			GameObject.FindObjectOfType<Death_Race_Camera>().SetPlayerCar(newVehicle);
		}
		...
		if (GameObject.FindObjectOfType<RCC_CustomizerExample> ()) {
			GameObject.FindObjectOfType<RCC_CustomizerExample> ().car = newVehicle.GetComponent<Death_Race_CarControllerV3> ();
