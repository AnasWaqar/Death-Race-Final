...
public class RCC_FixedCamera : MonoBehaviour {
	...
	private Death_Race_Camera rccCamera;
	...
	void Start(){
		...
		rccCamera = GameObject.FindObjectOfType<Death_Race_Camera> ();
	...
	void LateUpdate(){
		...
		targetPosition += currentCar.rotation * Vector3.forward * (currentCar.GetComponent<Death_Race_CarControllerV3>().speed * .05f);
		...
		transform.Translate ((-currentCar.forward * currentCar.GetComponent<Death_Race_CarControllerV3>().speed) / 50f * Time.deltaTime);
