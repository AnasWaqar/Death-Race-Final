...
public class Improved_Tracer_Enemy : MonoBehaviour {
	...
	void Start (){
		...
		Shooter = GameObject.FindGameObjectWithTag("ShooterEnemy");
	...
	void Update (){
		...
		if (Shooter != null) {
			...
			if (TargetObject != null){
				...
				//Target =GetComponent<LockOnTarget>().enemiesOnScreen[0].transform.position;
				//Target=GameObject.FindObjectOfType<LockOnTarget>().enemiesOnScreen[FindObjectOfType<LockOnTarget>().index].transform.position;
